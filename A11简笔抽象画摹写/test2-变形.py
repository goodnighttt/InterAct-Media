...
# 手动选择基准图像的特征点
base_points = np.array(select_feature_points(base_image, num_points=5), dtype=np.int32)
...
for i in range(1, len(images)):
    ...
    # 手动选择当前图像的特征点
    current_points = np.array(select_feature_points(current_image, num_points=5), dtype=np.int32)
    ...
    for triangle in triangles:
        ...
        # 进行形状变形
        warped_image = warp_image(base_image, base_triangle, current_triangle)

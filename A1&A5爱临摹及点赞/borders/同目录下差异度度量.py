...
def calculate_difference(img1, img2):
    ...
    thickness_diff = np.sum(np.abs(calculate_thickness(img1) - calculate_thickness(img2)))

    # 形状上下文差异
    # shape_context_diff = sd.computeDistance(calculate_ShapeContext_Distance(img1), calculate_ShapeContext_Distance(img2))

    # 返回每种差异度
    return pixel_diff, centroid_diff, gradient_direction_diff ,thickness_diff
    # , shape_context_diff
    # # 通过加权方式计算最终的差异度
    # # 这里的权重可以根据实际需要进行调整
...
# 获取目录下所有png文件的路径
image_files = glob.glob(os.path.join(image_dir, "*_binary_borders.png"))
...
# ref_img = cv2.imread('0_binary_borders.png', cv2.IMREAD_GRAYSCALE)
...
#     # 读取并处理图像
#     img = cv2.imread(f'{i}_binary_borders.png', cv2.IMREAD_GRAYSCALE)
#     _, img = cv2.threshold(img, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
#     contours, _ = cv2.findContours(img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
#     # 如果找到的连续区域足够多，取面积最大的作为cnt
#
#     cnt = max(contours, key=cv2.contourArea)
#     print(cnt)
#     if cnt.size > 0:
...
for image_file in image_files[1:]:
    ...
    pixel_diff, centroid_diff, gradient_diff,thickness_diff = calculate_difference(img_0, img)
    pixel_diffs.append(pixel_diff)
    centroid_diffs.append(centroid_diff)
    gradient_diffs.append(gradient_diff)
    thickness_diffs.append(thickness_diff)
...
thickness_diffs = (thickness_diffs - np.min(thickness_diffs)) / (np.max(thickness_diffs) - np.min(thickness_diffs))

print(pixel_diffs)
print(centroid_diffs)
print(gradient_diffs)
# print(shape_context_diffs)
print(thickness_diffs)
...
total_diffs = 0.25 * pixel_diffs + 0.25 * gradient_diffs + 0.25 * centroid_diffs + 0.25 * thickness_diffs
...
diff_dict = {
    ...
    'thickness_diffs': list(thickness_diffs),
...
for i, image_file in enumerate(image_files[1:], start=1):
    image_data.append({
        ...
        'thickness_diff': diff_dict['thickness_diffs'][i-1],
...
# 数据的列标题
column_names = ['image_number', 'image_name', 'pixel_diff', 'centroid_diff', 'gradient_diff', 'thickness_diff', 'total_diff']

# 创建一个新的CSV文件
with open('A1_image_diff_data.csv', 'w', newline='') as csvfile:
